...
mask = n_eur.links.index.to_series().apply(
    lambda x: "H2 pipeline" in x and any(k in x for k in keywords)
)

# Gefundene Links auflisten (zur Kontrolle)
print("Zu entfernende Links:")
print(n_eur.links.index[mask])

# Entfernen
n_eur.mremove("Link", n_eur.links.index[mask])
